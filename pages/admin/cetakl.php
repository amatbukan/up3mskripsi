...
$laporan = $_GET['lp'];
if($laporan == 'AND (tgl_serah_laporan = 0000-00-00)'){
	$lp =  "SELECT * FROM penelitian_pengabdian WHERE (jenis='0' OR jenis='1' OR jenis='2' OR jenis='3') AND (status='2' OR status='3' OR status='4') $laporan AND prodi='$prodi' ORDER BY id ASC";
}
else{
	$lp = "SELECT * FROM penelitian_pengabdian WHERE (YEAR(tgl_serah_laporan)='$tahun') AND (jenis='0' OR jenis='1' OR jenis='2' OR jenis='3') AND (status='5') $laporan AND prodi='$prodi' ORDER BY id ASC";
}
...
<table border="1">
			...
			$no=1;
			$jumlah = 0;
			$qO=mysql_query("$lp")or die(mysql_error());
	...
	if($jMM > 0){
		$nama1= $rMhs->mhs_nama;
			$qPr = mysql_query("SELECT * FROM prodi WHERE prodi_kode='$rMhs->mhs_prodi'");
			$rPr = mysql_fetch_object($qPr);
		$prodi= $rPr->prodi_nama;
	}elseif($jDosen > 0){
		$nama1= $rDosen->dosen_nama;
			$qPr2 = mysql_query("SELECT * FROM prodi WHERE prodi_kode='$rDosen->dosen_prodi'");
			$rPr2 = mysql_fetch_object($qPr2);
		$prodi= $rPr2->prodi_nama;
		}
	else{
		$nama1="";
		$prodi="";
	...
	?>			
			...
			<tr>
				...
				<th class="text-center"></th>
				<th class="text-center"></th>
				<th class="text-center"><?php echo rp($jumlah);?></th>
				<th class="text-center"></th>
				<th class="text-center"></th>
			</tr>
			</table>
